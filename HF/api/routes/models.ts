...
const KX_AI_MODELS: ModelEntry[] = [
  ...
  },
  {
    ...
    description: `ULTRAPLINIAN full tier: Race all ${ULTRAPLINIAN_MODELS.fast.length + ULTRAPLINIAN_MODELS.standard.length + (ULTRAPLINIAN_MODELS as any).ultra.length} models in parallel.`,
    tier: 'full',
    model_count:
      ULTRAPLINIAN_MODELS.fast.length +
      ULTRAPLINIAN_MODELS.standard.length +
      (ULTRAPLINIAN_MODELS as any).ultra.length,
  },
]

modelsRoutes.get('/', (_req, res) => {
  const data = KX_AI_MODELS.map(m => ({
    id: m.id,
...
modelsRoutes.get('/:model_id', (req, res) => {
  const entry = KX_AI_MODELS.find(m => m.id === req.params.model_id)
