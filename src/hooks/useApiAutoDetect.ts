...
/**
 * Auto-detect a self-hosted KX-AI API server at the same origin.
 *
    ...
    async function detect() {
      try {
        ...
        if (tierRes.ok || tierRes.status === 429) {
          ...
          console.log('[KX-AI] Self-hosted API detected (open auth) at', origin)
          ...
          setUltraplinianApiUrl(origin)
          console.log('[KX-AI] Self-hosted API detected (auth required) at', origin)
        }
      } catch {
